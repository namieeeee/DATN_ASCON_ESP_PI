...
/****************************************************************************************
 ...
 Frame format:
 [Frame Header][Function Code][Data Length][Data Content][Frame End]

 Frame Header: 1 byte, fixed value 0xF4
 Function Code: 1 byte, varies depending on function
 Data Length: total number of bytes including all fields
 Data Content: varies by function (see below)
 Frame End: 1 byte, fixed value 0xF5

          *****************Radar Reporting Protocol***********************  
 Direction: Radar ? Host Computer
 Function Code: 0x01 (Radar target information)

 Data Content format per target:
 [2 bytes Target X][2 bytes Target Y][1 byte Target Number][1 byte 0xF8]
 Repeat for each target

 Example:
 If radar detects a target at coordinates (1.5, 2.3), it reports:
 Frame Header: 0xF4
 Function Code: 0x01
 Data Length: number of targets * 6 bytes
 Data Content:
   Target 1:
     Target X: 2 bytes
     Target Y: 2 bytes
...
static bool parse_report(const uint8_t *buf, uint16_t len, HLK_Report *out)
{
    ...
    /* 15 byte: F4 F3 F2 F1 04 LL LH ... F8 F7 F6 F5 */
