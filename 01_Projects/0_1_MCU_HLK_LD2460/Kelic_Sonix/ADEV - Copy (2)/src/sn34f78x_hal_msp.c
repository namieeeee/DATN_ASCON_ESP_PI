...
extern volatile uint8_t btn_flag_1;
extern volatile uint8_t btn_flag_2;

extern HLK_CTX radar_ctx;
extern volatile uint8_t radar_rx;
extern volatile uint8_t fpga_rx_byte;
extern void FPGA_ProcessByte(uint8_t b);
...
{
    HAL_UART_Receive_IT(&UART1_Handle, (uint8_t *)&radar_rx, 1);
}
...
{
    static uint32_t last_tick_btn1 = 0;
    static uint32_t last_tick_btn2 = 0;
	  time_now();
    if (gpio == SN_GPIO1)
    {
        if (gpio_pin == GPIO_PIN_14) // BUTTON 1
        {
            if (now - last_tick_btn1 > 50)  // Debounce 50ms
            {
                btn_flag_1 = 1;
                last_tick_btn1 = now;
            }
        }
        else if (gpio_pin == GPIO_PIN_15) // BUTTON 2
        {
            if (now - last_tick_btn2 > 50)  // Debounce 50ms
            {
                btn_flag_2 = 1;
                last_tick_btn2 = now;
            }
...
void HAL_UART_RxCpltCallback(UART_Handle_T *huart)
{
    if (huart == &UART1_Handle) 
		{
        HLK_FeedByte(&radar_ctx, radar_rx);
        HAL_UART_Receive_IT(&UART1_Handle, (uint8_t *)&radar_rx, 1);
    }
		    if (huart == &UART2_Handle)  
    {
        FPGA_ProcessByte(fpga_rx_byte);
