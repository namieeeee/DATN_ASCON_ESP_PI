...
ERROR_CODE_T cbuffer_init(cbuffer_t *cb, void *buf, uint16_t size)
{
...
ERROR_CODE_T cbuffer_read(cbuffer_t *cb, uint8_t *buf, uint16_t bytes)
{
    ERROR_CODE_T ret = R_OK;  // Assume successful read unless buffer is empty
    uint16_t i;
    ...
    for (i = 0; i < bytes; i++)
    {
        ...
        else
        {
            ...
            ret = R_NG;  // Set return status to indicate no data available
            break;
        }
    }
...
ERROR_CODE_T cbuffer_only_read(cbuffer_t *cb, uint16_t idx, uint8_t *data)
{
    ERROR_CODE_T ret = R_OK;
...
ERROR_CODE_T cbuffer_clear(cbuffer_t *cb)
{
    ERROR_CODE_T ret = R_OK;
...
ERROR_CODE_T cbuffer_write(cbuffer_t *cb, uint8_t *buf, uint16_t bytes)
{
    ERROR_CODE_T ret = R_OK;  // Assume write successful
    uint16_t i;
    ...
    for (i = 0; i < bytes; i++)
    {
             ...
             (cb->index_read == 0)))                    // Wrap around to the start of the buffer
        {
            ...
            ret = R_ERROR;              // Set return status to indicate memory overflow
