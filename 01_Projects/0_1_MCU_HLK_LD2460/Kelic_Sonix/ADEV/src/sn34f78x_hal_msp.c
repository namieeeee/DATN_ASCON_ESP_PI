...
void HAL_GPIO_Callback(SN_GPIO_Type gpio, uint32_t gpio_pin)
{
    ...
    if (gpio == SN_GPIO1)
    {
        ...
        else if (gpio_pin == GPIO_PIN_15) // Nút 2
        {
...
{
  //  rx_flag++;
	    if (huart == &UART1_Handle) // radar uart
    {
        cbuffer_write(&cb_radar, &uart1_rx_byte, 1);
        /* restart RX IT */
        HAL_UART_Receive_IT(&UART1_Handle, &uart1_rx_byte, 1);
    }
}
